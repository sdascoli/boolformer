...
class Node():
    ...
    def treelib_prefix(self):
        tree = Tree()
        id = get_random_id()
        tree.create_node(self.value, id)
        for c in self.children:
            c._treelib_prefix(tree, id) 
        return tree
    
    def _treelib_prefix(self, tree, parent_id):
        id = get_random_id()
    ...
    def simplify(self):
        ...
        if self.value == 'and' and self.children[0].value=='not' and self.children[1].value=='not':
            self.value = 'not'
            self.children = [Node('or', self.params, [self.children[0].children[0], self.children[1].children[0]])]
        if self.value == 'or' and self.children[0].value=='not' and self.children[1].value=='not':
            self.value = 'not'
            self.children = [Node('and', self.params, [self.children[0].children[0], self.children[1].children[0]])]
