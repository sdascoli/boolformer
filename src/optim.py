...
class AdamWithWarmup(Adam):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, warmup_updates=10000, warmup_init_lr=1e-7):
        super().__init__(
            params,
            lr=warmup_init_lr,
            betas=betas,
...
class AdamInverseLogWithWarmup(Adam):
    """
    Decay the LR based on the inverse log of the update number.
    We also support a warmup phase where we linearly increase the learning rate
    from some initial learning rate (`warmup-init-lr`) until the configured
    learning rate (`lr`). Thereafter we decay proportional to the number of
    updates, with a decay factor set to align with the configured learning rate.
    During warmup:
        ...
        lr = lrs[update_num]
    After warmup:
        lr = decay_factor / log(update_num)
    where
        decay_factor = lr * log(warmup_updates)
    """
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, warmup_updates=10000, warmup_init_lr=1e-7):
        super().__init__(
        ...
        # then, decay prop. to the inverse square root of the update number
        self.decay_factor = warmup_end_lr * math.log(warmup_updates+1)
    ...
    def get_lr_for_step(self, num_updates):
        if num_updates < self.warmup_updates:
            ...
            return self.decay_factor / math.log(num_updates+1)
...
class AdamCosineWithWarmup(Adam):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, warmup_updates=10000, warmup_init_lr=1e-7,
                 min_lr=1e-9, init_period=100000, period_mult=1, lr_shrink=0.75, smooth=False):
        super().__init__(
            params,
            lr=warmup_init_lr,
            betas=betas,
...
def get_optimizer(parameters, s):
    ...
    if method == 'adadelta':
        ...
        optim_params.pop('beta1', None)
        optim_params.pop('beta2', None)
    elif method == 'adam_inverse_log':
        optim_fn = AdamInverseLogWithWarmup
        optim_params['betas'] = (optim_params.get('beta1', 0.9), optim_params.get('beta2', 0.999))
        optim_params.pop('beta1', None)
        optim_params.pop('beta2', None)
